...
def eotorath_dtd():
    ...
    if (args1, args2) == (no_args1, no_args2):
        ...
        # Check if they are valid or not before writing
        # Search the exact skills in the arguments
        args1 = (tools.matching_tools(args1)+["default"])[0]
        args2 = ([x for x in skills_list if args2.lower().replace(' ', '') in x.lower()]+["default"])[0]
    ...
    # Roll the skill checks along with bonus
    if bonus1 == 0:
	    skill_roll1 = vroll(tools.tool_check_dice_str(args1, base_adv=adv_table[adv1]))
	else:
	    bonuses = ""
	    for bonus in bonus1:
	        bonuses += "+" + bonus
	    skill_roll1 = vroll(f"{tools.tool_check_dice_str(args1, base_adv=adv_table[adv1])} {bonuses}")
